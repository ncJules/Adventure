...
class Room:
    ...
    def __init__(self, description):
        ...
        # Copy class Groups to instance variables
        for k, v in vars(type(self)).items():
...
class Group(set):
    ...
    def find(self, name):
        """Find a character in the group by name.
        Return None if the name does not match.
        """
        for character in self:
            if name.lower() in character.aliases:
                return character
        return None

    def __contains__(self, v):
        """Return True if a character is present in the group.
        If v is a str, then find the item by name, otherwise find the item by
        identity.
        """
        if isinstance(v, str):
